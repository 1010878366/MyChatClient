...
IPLineEdit::IPLineEdit(QWidget *parent):QLineEdit(parent)
{
    ...
    for (int i = 0;i<IP_INPUT_SIZE;i++)
    {
        ...
        m_lineEdit[i]->setFrame(false); //保存线条编辑是否使用边框绘制本身
        m_lineEdit[i]->setMaxLength(3); //输入框最大位数
        m_lineEdit[i]->setAlignment(Qt::AlignCenter); //消除布局中的控件,居中
        m_lineEdit[i]->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);    //保存小部件的默认布局

        m_lineEdit[i]->setValidator(new QRegExpValidator(regExp,this)); //添加验证规则
        m_lineEdit[i]->installEventFilter(this);   //安装事件过滤器 过滤子控件事件 截获控件按键 鼠标事件
